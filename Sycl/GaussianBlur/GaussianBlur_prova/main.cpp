...
int main() {
    ...
    // Caricamento originale (Interleaved)
    ...
    // --- CONVERSIONE DATA LAYOUT: INTERLEAVED -> PLANAR ---
    // Questo è il passaggio chiave suggerito dall'utente.
    // Invece di RGBRGB... usiamo RRR...GGG...BBB...
    int num_pixels = width * height;
    std::vector<float> input_planar(input.size());
    
    for (int i = 0; i < num_pixels; i++) {
        for (int c = 0; c < channels; c++) {
            input_planar[c * num_pixels + i] = input[i * channels + c];
        }
    }
    
    // Creiamo un output buffer planare per coerenza
    std::vector<float> output_planar(input.size(), 0.f);

    queue q = choose_and_printDevices(useCpu, print);
    
    // Usiamo i vettori planari per creare i buffer
    buffer<float, 1> buf_in(input_planar.data(), range<1>(input_planar.size()));
    buffer<float, 1> buf_out(output_planar.data(), range<1>(output_planar.size()));
    ...
    q.submit([&](handler& h) {
        ...
        h.parallel_for(range<2>(height, width), [=](id<2> idx) {
            int y = idx[0];
            int x = idx[1];
            int plane_size = width * height; // Dimensione di un piano (tutti i pixel di 1 canale)
            ...
            for (int c = 0; c < channels; c++) {
                ...
                for (int ky = -radius; ky <= radius; ky++) {
                    for (int kx = -radius; kx <= radius; kx++) {
                        ...
                        // --- NUOVO ACCESSO PLANAR ---
                        // Accediamo a [OffsetCanale + OffsetPixel]
                        // Questo rende l'accesso "coalesced" per thread adiacenti
                        int idx_in = c * plane_size + (ny * width + nx);

                        sum = sum + acc_in[idx_in] * acc_kernel[(ky + radius) * kerDim + (kx + radius)];
                    }
                }

                // Scrittura output in formato Planar
                int idx_out = c * plane_size + (y * width + x);
                acc_out[idx_out] = sum;
            }
        });
    });
    ...
    // --- RICONVERSIONE DATA LAYOUT: PLANAR -> INTERLEAVED ---
    // Dobbiamo rimettere i dati in RGBRGB per salvarli con stbi
    host_accessor host_out(buf_out, read_only);
    for (int i = 0; i < num_pixels; i++) {
        for (int c = 0; c < channels; c++) {
            // Leggiamo dal planare, scriviamo nell'interleaved
            output[i * channels + c] = host_out[c * num_pixels + i];
        }
    }
