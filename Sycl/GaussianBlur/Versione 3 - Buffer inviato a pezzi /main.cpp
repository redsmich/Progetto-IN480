...
int main() {
    ...
    for (int y_start = 0; y_start < height; y_start += chunk_height) {
        
        int current_chunk = std::min(chunk_height, height - y_start);
        ...
        q.submit([&](handler& h) {
            ...
            h.parallel_for(range<2>(current_chunk, width), [=](id<2> idx) {
                ...
                /*auto clampCoord = [](int coord, int maxVal) -> int {
                    if (coord < 0) return 0;
                    if (coord >= maxVal) return maxVal - 1;
                    return coord;
                };*/
